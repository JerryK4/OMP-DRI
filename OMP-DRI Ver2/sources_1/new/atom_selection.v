...
//(
//    parameter DW      = 24,    // ??nh d?ng 24-bit (Q10.13) cho ZedBoard
...
//    // Tham s? c?u hình (Th??ng c? ??nh cho 16x16)
//    input  wire [COL_W-1:0] N_cols, // VD: 255 (duy?t 256 c?t)
//    input  wire [ROW_W-1:0] M_rows, // VD: 15 (duy?t 16 hàng ~ 64 ph?n t?)

//    // Giao ti?p BRAM Ma tr?n Phi (Bus 96-bit)
//    output wire [ADDR_W-1:0] phi_addr,
//    input  wire [4*DW-1:0]   phi_data,

//    // Giao ti?p BRAM Th?ng d? r (Bus 96-bit)
//    output wire [ROW_W-1:0]  r_addr,
//    input  wire [4*DW-1:0]   r_data,

//    // Masking Control (Ki?m soát t?p Support Set ?ã ch?n)
//    input  wire [$clog2(MAX_I):0]  current_i,
//    input  wire [MAX_I*HIST_W-1:0] lambda_history,

//    // Output k?t qu?
//    output wire [COL_W-1:0] lambda_out, // Ch? s? Lambda t?t nh?t c?a vòng này
//    output wire             atom_done   // Báo hi?u ?ã tìm xong
//);

//    /* --- Tín hi?u dây k?t n?i n?i b? --- */
...
//       C?u hình Latency = 1 ?? kh?p v?i Single Port RAM
//    ========================================================== */
//    dot_product_4mac #(
//        .ADDR_W(ADDR_W),
//        .ROW_W(ROW_W),
...
//        .ROW_N(ROW_N),
//        .DW(DW),
//        .ACC_W(56),      // Tích l?y 56-bit cho 24-bit multiplier
//        .OUT_W(DOT_W)    // 48-bit
//    ) u_parallel_mac (
//        .clk(clk),
//        .rst_n(rst_n),
...
//       L?y k?t qu? t? MAC và tìm ra Max th?a mãn không b? Mask
//    ========================================================== */
//    finding_max #(
//        .DOT_W(DOT_W),   // 48-bit
//        .COL_W(COL_W),   // 8-bit
